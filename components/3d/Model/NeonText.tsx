...
import { Text3D, OrbitControls, PivotControls, useFont } from '@react-three/drei';
...
function curveParamsForName(len: number, t: TweakState) {
  if (len <= 5)
    return {
      radius: t.curveRadiusShort,
      sag: t.curveSagShort,
      tilt: t.curveTiltShort,
      cx: t.curveXShort,
      cy: t.curveYShort,
      cz: t.curveZShort,
    };
  ...
  if (len <= 11)
    return {
      radius: t.curveRadiusLong,
      sag: t.curveSagLong,
      tilt: t.curveTiltLong,
      cx: t.curveXLong,
      cy: t.curveYLong,
      cz: t.curveZLong,
    };
  return {
    radius: t.curveRadiusXLong,
    sag: t.curveSagXLong,
    tilt: t.curveTiltXLong,
    cx: t.curveXXLong,
    cy: t.curveYXLong,
    cz: t.curveZXLong,
  };
...
function textWidth(fontData: any, chars: string[], size: number, extraGap: number) {
...
function nameNum(t: TweakState, kind: 'Size' | 'X' | 'Y' | 'Z', len: number): number {
...
function glyphAdvance(fontData: any, ch: string, size: number): number {
  const g =
    fontData?.glyphs?.[ch] || fontData?.glyphs?.[ch.toUpperCase()] || fontData?.glyphs?.['?'];
  const ha = g?.ha ?? 700;
...
function DebugPanel({ sport, isNumber, len }: { sport: string; isNumber: boolean; len: number }) {
  ...
  useEffect(() => {
    ...
    const checkbox = (label: string, key: keyof TweakState, values: TweakState) => (
      <label
        key={key}
        style={{
          display: 'flex',
          ...
          {openVal && (
            <>
              <p style={{ fontSize: 10, color: '#888', margin: '0 0 8px' }}>
                Curve = basketball names only. Letter gap is constant. Copy values into DEFAULTS
                when done.
              </p>
              ...
              <Section title="Basketball curve">
                <details style={{ marginLeft: 8, marginBottom: 6 }}>
                  <summary
                    style={{ cursor: 'pointer', fontSize: 10, color: '#aaa', marginBottom: 4 }}
                  >
                    Short (1-5 chars)
                  </summary>
              ...
              <Section title="Name position (by character count)">
                {Array.from({ length: 13 }, (_, i) => (
                  <details key={i} open={len === i + 1} style={{ marginLeft: 8, marginBottom: 6 }}>
                    <summary
                      style={{ cursor: 'pointer', fontSize: 10, color: '#aaa', marginBottom: 4 }}
                    >
                      {i + 1} char{i ? 's' : ''}
...
export default function NeonText({
  ...
  const baseSize = isNumber ? sizeForNumber(len, sport, t) : sizeForName(len, sport, t);
  ...
  const size = useMemo(() => {
    ...
    const total = widths.reduce((a, b) => a + b, 0) + extraGap * Math.max(0, len - 1);
  ...
  const activeIntensity = neonOn ? (isSoft ? t.softIntensity : t.intensity) : t.offIntensity;
  ...
  const offsets = isNumber
    ? offsetParamsForNumber(len, sport, t)
    : offsetParamsForName(len, sport, t);
  ...
  const renderCurved = () => {
    ...
    const xMids = chars.map(c => {
      const g =
        fontData?.glyphs?.[c] || fontData?.glyphs?.[c.toUpperCase()] || fontData?.glyphs?.['?'];
  ...
  const content = (
    <group position={[position[0] + ox, position[1] + oy, position[2] + oz]} scale={sc}>
      {useCurve ? renderCurved() : renderFlat()}
    </group>
  );

  const debug = DEBUG.panel ? <DebugPanel sport={sport} isNumber={isNumber} len={len} /> : null;
